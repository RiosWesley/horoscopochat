...
// --- Function to Create Initial Analysis Record ---
export const createInitialAnalysisRecord = functions.https.onCall(async (data: unknown, context) => {
  functions.logger.info("createInitialAnalysisRecord function started.");
  // No specific payload needed, just create a placeholder
  try {
    const db = admin.firestore();
    const initialData = {
      isPremiumAnalysis: false,
      createdAt: admin.firestore.FieldValue.serverTimestamp(),
      // Add any other minimal default fields if necessary
      placeholder: true, // Flag to indicate this is just a placeholder
    };
    const docRef = await db.collection("sharedAnalyses").add(initialData);
    functions.logger.info(`Initial analysis record created with ID: ${docRef.id}`);
    return { success: true, analysisId: docRef.id };
  } catch (error) {
    functions.logger.error("Error creating initial analysis record:", error);
    const errorMessage = error instanceof Error ? error.message : "Unknown error";
    throw new functions.https.HttpsError("internal", `Failed to create initial analysis record: ${errorMessage}`);
  }
});
...
interface CreateCheckoutPayload {
  analysisId: string;
  title: string;
  unitPrice: number; // Expect price in BRL cents (e.g., R$ 5.00 = 500)
  successUrl: string; // URL to redirect after successful payment
  failureUrl: string; // URL to redirect after failed payment
  pendingUrl: string; // URL to redirect for pending payment
}

export const createMercadoPagoCheckout = functions.https.onCall(async (data: unknown, context) => {
  ...
  const { analysisId, title, unitPrice, successUrl, failureUrl, pendingUrl } = requestData as CreateCheckoutPayload;
  ...
  try {
    ...
    const preferenceData = await preference.create({
      body: {
        ...
        notification_url: `https://us-central1-horoscopozap.cloudfunctions.net/handleMercadoPagoNotification`, // Updated with actual deployed URL
    ...
    throw new functions.https.HttpsError(
      statusCode === 400 ? "invalid-argument" : "internal",
      `Falha ao criar checkout (${statusCode}): ${mpErrorMessage}`
    );
  }
});
...
import { Payment } from "mercadopago"; // Import Payment type

export const handleMercadoPagoNotification = functions.https.onRequest(async (request, response) => {
